...
- [ ] **カプセル半径の自動推定** — 素体メッシュから半径を推定。**v1 landed**(最近接軸割り当て+パーセンタイル+外れ値ゲート; 胴/首/脚/上腕はアバター別の妥当値へ、胴は固定0.12→約0.06で誤検出消滅)。残課題: 前腕/肩/胸が**隣部位の頂点混入**で過大(global パーセンタイルでは根治不可)→ 次は**スキニング基準割り当て**(各カプセルを自ボーンにウェイトが乗る頂点だけから測る)
