...
- **バスト・ヒップ = 軸方向の周径プロファイルが極大になる点** — 胸・尻の出っ張りを、ボーン区間の代表値でなく**断面輪郭が外へ膨らんで戻る点(接線の傾きが反転する極値)**で取る。胴帯(Hips〜Chest)を軸方向にスライスして周径(取り方は §8)をプロファイル化し極大を検出。
...
**実装 [landed 2026-07-01]**: §8 のスライス周径プロファイルを純 Core `GirthProfile`(軸方向バンド分け→セクタ最遠点を角度順に結ぶ外輪郭周長→平滑化→prominence ゲート付き極値検出、ユニット 10 件)として実装。`GirthProfile.ClassifyTorso` が胴帯の極値を **hips=最下 t の極大 / bust=最上 t の極大 / waist=その間の最小** に命名。エディタ glue `VRClothGirthMeasure` が Humanoid の **Hips→(UpperChest∥Chest∥Spine)軸**(バストが内側に入るよう上端を 15% 延長)を解決し body 頂点を渡す。採寸ダンプ(§3)が body 採寸行に `girth`(bust/waist/hips の実周径 m＋軸位置)を載せ schema `/3` へ。**計測条件**: バスト/ヒップの極大はシェイプキーで動くので shapekey 0 を固定(§6)。プロファイルは平滑化＋prominence で誤検出(単純 argmax)を避ける。下流の `tools/sizing_table_export.py` が `girth.measured` 時に計測点名 `points: {bust,waist,hips}` を採寸表 `vrcloth-sizing-table/1` に出す。**残**: 四肢の定位置サンプル(上腕/前腕/太もも/脹脛)、しきい値(band 数・prominence・軸延長)の **E2E 較正**(現状は未較正の暫定既定)。
...
**JSONL 1行スキーマ(schema `vrcloth-body-measurement/3`)**:
```
{ schema, timestamp, avatar, headCount_neckRef, headCount_headRef, height_m,
  bodyCoverage, meshHash, conditions:{ radiusPercentile },
  capsuleCount, capsules:[ { label, radius_m, length_m, sampleCount, estimated }, … ],
  girth:{ measured, bust_girth_m, waist_girth_m, hips_girth_m,
          bust_axisT, waist_axisT, hips_axisT } }   // §2 標準計測点(/3 で追加)
```
版: `/1`→`/2`(`meshHash`＋`conditions` 追加, §6)→`/3`(`girth` 標準計測点 追加, §2)。
...
ECOSYSTEM_VISION §6 の identity。各採寸行に**何を測ったか**を固定する。誤差を「計測条件と実運用条件の不一致(condition mismatch)」として主題化し、下の保留(条件の完全化)を概念から裏づけるのが [MEASUREMENT_CONDITIONS.md](MEASUREMENT_CONDITIONS.md)。
...
- **半径=円断面近似**。真の周径への置き換えは §2 の精度余地。**周径の取り方(2026-06-26, ユーザー提案)**: 軸周りの**実頂点を結んだ周長**で取る(凸包は胴のくびれ等の凹みを埋めてしまうため避ける)。理想は Blender のエッジループ(Alt+辺選択)的な水平リングだが、**エッジループは quad メッシュ前提**で、VRChat アバターは Unity 用に triangulate 済み=対向エッジが無くループ選択が破綻する。**堅牢版 = 軸方向の薄いバンドに頂点をビン分け → 軸周りに角度ソート → 順に結んだ周長**(トポロジ非依存・実頂点・凹凸反映・スライス交差計算も不要)。留保: 角度ソートは断面が概ね凸(人体の胴/腕/脚は概ね凸)なら正確、複雑断面(装飾が混じる等)では順序が乱れるので平滑化/ロバスト化が要る。バスト/ヒップの極値(最大周長バンド)検出と組み合わせる。
...
- **landed**: アバター body 採寸(分割素体合算・偽 GREEN ガード・dump ボタン・ヘッドレス CLI(a)(b))。実証: unkt × 所持素体4体を OMs でヘッドレス採寸し最近傍=Shinano を算出。**標準計測点(§2, 2026-07-01)**: 軸方向周径プロファイル `GirthProfile`＋胴帯のバスト/ウエスト/ヒップ命名を body 採寸行に配線(schema `/3`)。ユニット 10 件緑(EditMode 全 156 緑)。**残=実機での較正と四肢点**。
- **次**: ① 標準計測点を実アバターで較正(band 数・prominence・軸延長, §2 残)＋四肢の定位置サンプル → ② アバター全素体 BASE を採寸 → SQLite 取込 → クラスタ分析 → ③ 衣装内周 measure(§4)→ ④ マッチング行列(§7)。各採寸行に `meshHash`＋条件(§6)を刻む。
